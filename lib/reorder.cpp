...
int reorder_dddmp_file(DdManager *mgr, const std::string &in_path, const std::string &out_path) {
    ...
    // assert(res_0); Cudd_Ref(res_0);
    // const auto res_1 = Cudd_bddOr(mgr, res_0, mt2);
    // assert(res_1); Cudd_Ref(res_1);
    // const auto res_2 = Cudd_bddOr(mgr, res_1, mt3);
    // assert(res_2); Cudd_Ref(res_2);
    // const auto res = res_2;
    // const auto res = Cudd_ReadOne(mgr);
    // const auto res = Cudd_ReadLogicZero(mgr);
    const auto res = Cudd_bddXor(mgr, a, b);
    Cudd_Ref(res);
    assert(res);
    fprintf(stderr, "chk2\n");
    Cudd_DebugCheck(mgr);
